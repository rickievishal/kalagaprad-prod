...
const AppointmentForm = ({userInfo}) => {
    ...
    const [formData,setFormData] = useState({
        ...
        timeSlot : "",
        city : "mumbai",
        pickedDate : getDateThreeDaysFromToday(),
        appointmentType : "",
        price : 0,
        paymentStatus : false,
        bookingId : uuid()
  ...
  return (
    <div className='w-full my-8'>
            ...
            <div className='w-full max-w-sm sm:max-w-xl lg:max-w-4xl  mx-auto px-2 pb-8'>
                {
                    step === 1 && (<>
                            <div className='w-full flex flex-col sm:flex-row justify-center gap-2'>
                                <div className={`w-full flex justify-start items-start py-4 px-2 border border-white/20 ${appointmentType === "phone" && ("bg-[rgb(152,58,207)]")} bg-[rgba(142,53,194,0.33)] hover:bg-[rgb(142,53,194)] rounded-lg hover:cursor-pointer`}
                                    onClick={()=>handleAppointmentSelect("phone")}>
                                    <div className=' min-w-[20px] min-h-[20px] w-[20px] h-[20px]  rounded-full border p-1 ' >
                                    ...
                                    <div className='pl-4 flex flex-col leading-[105%]'>
                                        ...
                                        <p className='mt-1 text-sm text-gray-300'>
                                            You get a 30 mins Whatsapp call for 1000₹
                                        </p>
                                    </div>
                                </div>
                                <div className={`w-full flex justify-start items-start py-4 px-2 border border-white/20 ${appointmentType === "video" && ("bg-[rgb(152,58,207)]") } bg-[rgba(142,53,194,0.33)] hover:bg-[rgb(142,53,194)] rounded-lg hover:cursor-pointer`}
                                onClick={()=>handleAppointmentSelect("video")} >
                                    ...
                                    <div className='pl-4 flex flex-col leading-[105%]'>
                                        ...
                                        <p className='mt-1 text-sm text-gray-300'>
                                            You get a 30min video call for the 1000₹
                                        </p>
                                    </div>
                                </div>
                                <div className={`w-full flex justify-start items-start py-4 px-2 border border-white/20 ${appointmentType === "offline" && ("bg-[rgb(152,58,207)]") } bg-[rgba(142,53,194,0.33)] hover:bg-[rgb(142,53,194)] rounded-lg hover:cursor-pointer`}
                                onClick={()=>handleAppointmentSelect("offline")}>
                                    ...
                                    <div className='pl-4 flex flex-col leading-[105%]'>
                                        ...
                                        <p className='mt-1 text-sm text-gray-300'>
                                            You get a offline appointment for the 1500₹
                                        </p>
                                    </div>
                                </div>
                            </div>
                    ...
                    step === 2 && (
                        <>
                            <div className='w-full grid grid-cols-3 gap-x-4'>
                                ...
                                <div className='col-span-3 lg:col-span-2 mt-4 lg:mt-0 border border-black/20 p-2 rounded-lg'>
                                    <h2>
                                        Time Slot
                                    </h2>
                                    <div className='grid grid-cols-2 gap-2 mt-4 max-h-[300px] overflow-auto'>
                                        {
                                            timeSlots.map((timeslot)=> (
                                                <div className={`col-span-1 border border-purple-300 rounded-lg h-[50px] text-sm flex justify-center items-center px-2 ${timeslot.isAvailable ? ("hover:bg-purple-600 hover:cursor-pointer") :("opacity-35")} ${timeslot.time === formData.timeSlot && ("border-purple-500 bg-purple-500")}`} 
                                                    onClick={()=>{
                                                        if(timeslot.isAvailable){
                                                            handleTimeSelect(timeslot.time)
                                                        }
                                                    }}>
                                                    <IoMdTime  className='text-lg sm:text-xl' /> <p className='pl-1 text-sm text-wrap'>{timeslot.time} </p>
                                                </div>   
                                            ))
                                        }  
                ...
                {
                    step === 3 && (<>
                    <div className="max-w-2xl mx-auto w-full bg-white border border-purple-300 rounded-xl shadow-lg p-6">
                        <h2 className="text-2xl font-bold text-purple-700 border-b border-purple-200 pb-3">
                            Appointment Summary
                        </h2>
                        <div className="mt-4 space-y-4">
                            <div className="flex justify-between items-center text-gray-500">
                            <span className="text-gray-500">Name</span>
                            <span className="font-semibold capitalize">{formData.name}</span>
                            </div>
                            <div className="flex justify-between items-center text-gray-500">
                            <span className="text-gray-500">Appointment Type</span>
                            <span className="font-semibold">{formData.appointmentType}</span>
                            </div>
                            <div className="flex justify-between items-center text-gray-500">
            ...
            <div className='w-full max-w-sm lg:max-w-4xl mx-auto flex justify-between px-2'>
                    {
                        step !== 1 && (
                            <Button className={"py-2 bg-black hover:bg-[rgb(39,39,39)]"} onClick={handlePrev}>
                                Previous
                            </Button>
                        )
                    }
                    {
                        step === 1 && (
                            <Button className={"py-2 bg-black hover:bg-[rgb(39,39,39)]"} onClick={()=>navigate("/")}>
                                Cancel
                            </Button>
                            )
                    }
                    ...
                    {
                        step === 3 && (<PaymentComponent formData={formData} userInfo={user} paymentAmount={formData.price}/>)
                    }
            </div>
    </div>
  )
